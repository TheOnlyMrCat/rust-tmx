...
impl<'de> Visitor<'de> for PointListVisitor {
    ...
    fn visit_str<E>(self, value: &str) -> Result<Vec<(i32, i32)>, E> where E: de::Error {
        let mut failed = false;
        let v = value.split(' ').map(|x| {
...
where D: Deserializer<'de> {
    ...
    match StringOrInt::deserialize(deserializer)? {
        StringOrInt::String(s) =>  s.parse::<f32>().map(|x| x.round() as i32).map_err(serde::de::Error::custom),
